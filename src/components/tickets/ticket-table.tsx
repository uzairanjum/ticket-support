...
import { Ticket as TicketIcon, User, Info } from 'lucide-react';
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
    ...
    if (tickets.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-16 border rounded-xl bg-card/50 border-dashed">
                ...
                <p className="text-xs text-muted-foreground/70 mt-1">Tickets will appear here when created</p>
            </div>
        );
    }
                    ...
                    <Table className="w-full table-fixed min-w-[800px]">
                        <TableHeader>
                            <TableRow className="bg-muted/30 hover:bg-muted/30">
                                ...
                                {showCustomer && <TableHead className="w-[200px] font-semibold">Customer</TableHead>}
                                {showStaff && <TableHead className="w-[180px] font-semibold">Assigned</TableHead>}
                                <TableHead className="w-[120px] font-semibold">Created</TableHead>
                                {actions && <TableHead className="w-[100px] text-right font-semibold">Actions</TableHead>}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {tickets.map((ticket) => (
                                <TableRow key={ticket.id} className="group transition-colors hover:bg-muted/20">
                                    <TableCell className="font-mono text-xs text-muted-foreground font-medium whitespace-normal">
                                        #{ticket.id}
                                    </TableCell>
                                    <TableCell className="whitespace-normal">
                                        <div className="w-full overflow-hidden">
                                            <TypeBadge type={ticket.ticket_type} className="max-w-full" />
                                        </div>
                                    </TableCell>
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <div className="flex items-center gap-2.5 cursor-help overflow-hidden">
                                                            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold shadow-sm shrink-0">
                                                                {getInitials(ticket.customer.name)}
                                                            </div>
                                                            <div className="flex flex-col min-w-0">
                                                                <span className="text-xs font-semibold truncate">{ticket.customer.name}</span>
                                                                <span className="text-[10px] text-muted-foreground truncate">{ticket.customer.email}</span>
                                                            </div>
                                                        </div>
                                                    </TooltipTrigger>
                                                    <TooltipContent side="bottom">
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-[10px] opacity-80">{ticket.customer.email}</p>
                                                        </div>
                                                    </TooltipContent>
                                                </Tooltip>
                                            ) : (
                                        ...
                                        <TableCell className="whitespace-normal">
                                            {ticket.staff ? (
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <div className="flex items-center gap-2.5 cursor-help overflow-hidden">
                                                            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white text-[10px] font-bold shadow-sm shrink-0">
                                                                {getInitials(ticket.staff.name)}
                                                            </div>
                                                            <span className="text-xs font-medium truncate">{ticket.staff.name}</span>
                                                        </div>
                                                    </TooltipTrigger>
                                                    <TooltipContent side="bottom">
